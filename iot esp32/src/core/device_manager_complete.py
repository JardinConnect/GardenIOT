...
class DeviceManager:
    ...
    def _format_sensor_data(self, sensor_data):
        """
        Format sensor data for transmission using DTO compact format.
        """
        from models.sensor_data import SensorData
        
        # Convert sensor data to DTO format
        dto_data = {}
        for sensor_name, metrics in sensor_data.items():
            ...
            dto_data[sensor_name] = dto.to_compact()
        
        # Format as "code:value,code:value,..."
        formatted_parts = []
        for sensor_name, compact_data in dto_data.items():
