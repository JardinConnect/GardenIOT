...
def parse_protocol_message(message):
    """
    Parse messages in format: B|TYPE|TIMESTAMP|UID|DATAS|E
    where DATAS is like: 1B85:1TA21:1TS22:1HA65:1HS45:L50
    """
    try:
        ...
        if msg_type == MSG_TYPE_DATA:
            ...
            for part in data_parts:
                ...
                # Parse each sensor data (e.g., "1B85", "1TA21", "L50")
                sensor_id = None
                sensor_type = None
                value = None
...
def format_parsed_data(parsed):
    ...
    if parsed['type'] == 'DATA':
        for key, data in parsed['sensors'].items():
            ...
            output.append(f"   • {data['type']} {sensor_label}: {data['value']}%")
...
print("📡 Waiting for protocol messages...")
print("-" * 40)
