...
#        gpio_interrupt = Pin(self._interrupt, Pin.IN, Pin.PULL_DOWN)
        ...
        assert self._spi_read(REG_01_OP_MODE) == (MODE_SLEEP | LONG_RANGE_MODE), \r
        ...
        for _ in range(retries + 1):
            ...
            while time.time() - start < self.retry_timeout + (self.retry_timeout * (getrandbits(16) / (2**16 - 1))):
                if self._last_payload:
                    if self._last_payload.header_to == self._this_address and \r
                            self._last_payload.header_flags & FLAGS_ACK and \r
    ...
    def _decrypt(self, message):
        decrypted_msg = self.crypto.decrypt(message) # type: ignore
    ...
    def _encrypt(self, message):
        ...
        encrypted_msg = self.crypto.encrypt(msg_bytes) # type: ignore
        return encrypted_msg

    def _handle_interrupt(self, channel):
        irq_flags = self._spi_read(REG_12_IRQ_FLAGS)

        if self._mode == MODE_RXCONTINUOUS and (irq_flags & RX_DONE): # type: ignore
            ...
            packet = self._spi_read(REG_00_FIFO, packet_len) # type: ignore
            self._spi_write(REG_12_IRQ_FLAGS, 0xff)  # Clear all IRQ flags

            snr = self._spi_read(REG_19_PKT_SNR_VALUE) / 4 # type: ignore
            rssi = self._spi_read(REG_1A_PKT_RSSI_VALUE)

            if snr < 0:
                rssi = snr + rssi # type: ignore
            else:
                rssi = rssi * 16 / 15 # type: ignore
            ...
            if packet_len >= 4: # type: ignore
                header_to = packet[0] # type: ignore
                header_from = packet[1] # type: ignore
                header_id = packet[2] # type: ignore
                header_flags = packet[3] # type: ignore
                message = bytes(packet[4:]) if packet_len > 4 else b'' # type: ignore
        ...
        elif self._mode == MODE_TX and (irq_flags & TX_DONE): # type: ignore
            self.set_mode_idle()

        elif self._mode == MODE_CAD and (irq_flags & CAD_DONE): # type: ignore
            self._cad = irq_flags & CAD_DETECTED # type: ignore
            self.set_mode_idle()
