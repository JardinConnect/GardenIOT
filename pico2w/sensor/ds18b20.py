...
class DS18B20Sensor:
    ...
    def read_digital(self):
        """
        Lit la valeur numérique du capteur (froid/chaud)
        
        Returns:
            bool: True si chaud, False si froid
        """
        temp = self.read_temperature()
        if temp is None:
            return False
            
        # Seuil par défaut : température ambiante (20°C)
        threshold = (self.min_temp + self.max_temp) / 2 if self.max_temp > 20 else 20.0
        return temp > threshold
    
    def read_percent(self):
        """
        Lit la température en pourcentage (équivalent aux autres capteurs)
        
        Returns:
            float: Pourcentage de température (0-100%)
        """
        if self.use_percent:
        ...
        percent = ((temp - self.min_temp) / (self.max_temp - self.min_temp)) * 100
